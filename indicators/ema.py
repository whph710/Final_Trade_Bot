...
def _calculate_bonuses(ema9, ema21, ema50, price, current_ema9,
    ...
    if alignment == 'BULLISH' and price > current_ema9:
        ...
        if distance < config.EMA_DISTANCE_OPTIMAL_PCT:
            bonus += config.EMA_DISTANCE_OPTIMAL_BONUS

        if volume_ratio >= config.VOLUME_STRONG_THRESHOLD:
            bonus += config.VOLUME_STRONG_SCORE
...
def _calculate_penalties(ema21, volume_ratios, ema50, price, ema9_arr, ema21_arr):
    ...
    if ema21_slope < config.EMA_SLOPE_FLAT_THRESHOLD:
        penalty += config.EMA_SLOPE_FLAT_PENALTY

    distance = abs((price - ema50) / ema50 * 100)
    if distance > config.EMA_DISTANCE_FAR_PCT:
        penalty += config.EMA_DISTANCE_FAR_PENALTY

    recent_volume = [float(v) for v in volume_ratios[-3:]]
    if all(v < config.EMA_VOLUME_LOW_THRESHOLD for v in recent_volume):
        penalty += config.EMA_VOLUME_LOW_PENALTY
    ...
    if crosses >= config.EMA_CROSSES_CHOPPY_THRESHOLD:
        penalty += config.EMA_CROSSES_CHOPPY_PENALTY
