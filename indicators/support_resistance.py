...
def find_support_resistance_levels(
    ...
    Args:
        ...
        min_touches: Минимум касаний для уровня (3+)
        touch_tolerance_pct: Допуск касания (0.5%)
...
def _determine_price_zone(
    ...
    if nearest_support and nearest_support.distance_from_current_pct < 1.0:
        return 'NEAR_SUPPORT'

    if nearest_resistance and nearest_resistance.distance_from_current_pct < 1.0:
...
def _calculate_sr_adjustment(
    ...
    if signal_direction == 'LONG':
        if nearest_support:
            ...
            if nearest_support.distance_from_current_pct < 1.0:
                adjustment += 25

                # Бонус за сильный уровень
                if nearest_support.touches >= 4:
                    adjustment += 10
    ...
    # SHORT сигнал
        ...
        if nearest_resistance:
            ...
            if nearest_resistance.distance_from_current_pct < 1.0:
                adjustment += 25

                # Бонус за сильный уровень
                if nearest_resistance.touches >= 4:
                    adjustment += 10
