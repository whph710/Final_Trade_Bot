...
def find_imbalances(
        candles,
        lookback: int = None,
        min_gap_size_pct: float = None
) -> List[ImbalanceData]:
    """Найти Fair Value Gaps (имбалансы)"""
    from config import config
    
    if lookback is None:
        lookback = config.IMB_LOOKBACK
    if min_gap_size_pct is None:
        min_gap_size_pct = config.IMB_MIN_GAP_SIZE_PCT
...
def _check_gap_fill_improved(
    ...
    try:
        ...
        # 1. Максимальное проникновение > threshold, ИЛИ
        # 2. Суммарное проникновение > total threshold, ИЛИ
        # 3. Более N касаний зоны

        is_filled = (
            max_fill > config.IMB_FILL_THRESHOLD_PCT or
            total_penetration > config.IMB_FILL_TOTAL_THRESHOLD or
            touch_count > config.IMB_FILL_TOUCH_COUNT
        )
...
def _calculate_imbalance_adjustment(
    ...
    try:
        ...
        adjustment += config.IMB_BASE_ADJUSTMENT

        # Бонус за незаполненный imbalance
        if not nearest.is_filled:
            adjustment += config.IMB_UNFILLED_BONUS
        elif nearest.fill_percentage < config.IMB_PARTIAL_FILL_30_PCT:
            # Частично заполнен (<30%)
            adjustment += config.IMB_PARTIAL_30_BONUS
        elif nearest.fill_percentage < config.IMB_PARTIAL_FILL_50_PCT:
            # Частично заполнен (30-50%)
            adjustment += config.IMB_PARTIAL_50_BONUS

        # Penalty за большую дистанцию
        if nearest.distance_from_current > config.IMB_DISTANCE_FAR_PCT:
            adjustment += config.IMB_DISTANCE_FAR_PENALTY
        elif nearest.distance_from_current < config.IMB_DISTANCE_CLOSE_PCT:
            # Очень близко
            adjustment += config.IMB_DISTANCE_CLOSE_BONUS
