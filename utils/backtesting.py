...
class Backtester:
    ...
    async def _fetch_5m_candles_after_signal(
        ...
        """
        Загрузить 5-минутные свечи ПОСЛЕ момента сигнала
        Автоматически определяет тип актива и использует соответствующий провайдер
        ...
        Returns:
            Список 5M свечей ПОСЛЕ сигнала
        """
        try:
            ...
            # Универсальная функция fetch_candles автоматически определяет тип актива
            # Для crypto используем start_time, для stocks загружаем последние N свечей
            if asset_type == 'crypto':
                ...
                # Для stocks загружаем последние N свечей и фильтруем вручную
                candles_5m = await fetch_candles(
                    symbol,
                    interval='5',
                    limit=config.BACKTEST_CANDLES_LIMIT
            ...
            # (важно для stocks, где start_time не поддерживается напрямую)
            filtered_candles = []
